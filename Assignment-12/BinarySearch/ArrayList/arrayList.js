...
class ArrayList {
    ...
    contains(element) {
        if (this.capacity = initialCapacity) {
            let begin = 0;
            let end = this.storage.length;
            while (end - begin > 1) {
                let middle = Math.floor((begin + end) / 2);
                if (this.storage[middle] > element) {
                    end = middle;
                } else {
                    begin = middle;
                }
            }
            if (element === this.storage[begin]) {
                return begin;
            } else {
                return -1;
            }
            ...
            let begin = 0;
            let end = this.newstorage.length;
            while (end - begin > 1) {
                let middle = Math.floor((begin + end) / 2);
                if (this.newstorage[middle] > element) {
                    end = middle;
                } else {
                    begin = middle;
                }
            }
            if (element === this.newstorage[begin]) {
                return begin;
            } else {
                return -1;
            }
...
let nameArr = ["big", "small", "tall", "short", "round", "square", "enormous",
    "tiny", "gargantuan", "lilliputian", "numberless", "none", "vast",
    "miniscule"
];
let arrayList = new ArrayList(15);
console.log(arrayList.contains("big"))